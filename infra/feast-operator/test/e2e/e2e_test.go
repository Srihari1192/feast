...
var _ = Describe("controller", Ordered, func() {
	...
	BeforeAll(func() {
		...
		utils.CreateNamespace(namespace, "/test/e2e")
	})
	...
	AfterAll(func() {
		...
		utils.DeleteNamespace(namespace, "/test/e2e")
	})

	Context("Operator E2E Tests", func() {
		It("Should be able to deploy and run a TesDefaultFeastCR successfully", runTestDeploySimpleCRFunc)
		It("Should be able to deploy and run a TestRemoteRegistryFeastCR  successfully", runTestWithRemoteRegistryFunction)
	})
})
